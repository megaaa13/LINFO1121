...
public class WordTransformationSP {
    ...
    public static int minimalCost(String from, String to) {
        ...
        PriorityQueue<Object[]> queue = new PriorityQueue<>(Comparator.comparingInt(o -> (Integer) o[1]));
        queue.add(new Object[]{from, 0});
        distTo.put(from, 0);
        while(!queue.isEmpty()) {
            Object[] current = queue.poll();
            String currS = (String) current[0];
            for (int i = 0; i < currS.length(); i++) {
                for (int j = i + 2; j <= currS.length(); j++) {
                    String out = rotation(currS, i, j);
                    int cost = distTo.get(currS) + (j - i);
                    if (!distTo.containsKey(out) || distTo.get(out) > cost) {
                        distTo.put(out, cost);
                        queue.add(new Object[]{out, cost});
                    }
                }
            }
        }
