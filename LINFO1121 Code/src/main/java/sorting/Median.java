...
public class Median {
    ...
    public static int qsort(Vector vec, int lo, int hi) {
        int pivot = vec.get(hi), pivotloc = lo;
        for (int i = lo; i <= hi; i++) {
            if (vec.get(i) < pivot) {
                vec.swap(i, pivotloc);
                pivotloc++;
    ...
    public static int median(Vector vec, int lo, int hi) {
        int i = qsort(vec, lo, hi);
        if (i == vec.size()/2) return vec.get(i);
        else if (i > vec.size()/2) return median(vec, lo, i-1);
        else return median(vec, i+1, hi);
    }
