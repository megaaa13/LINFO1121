...
public class MinPQLinked<Key> {
    ...
    private Key removeLastNodeInLastLayer() {
        ...
        while(current.left != null) {
            if (!isPowerOfTwo(current.left.size+1) || current.right == null) {
                current = current.left;
            } else if (!isPowerOfTwo(current.right.size+1)) {
                    current = current.right;
            } else if (current.left.size > current.right.size) {
                current = current.left;
            } else {
                current = current.right;
            }
