...
public class BinarySearchTree {
    ...
    public static Integer ceil(BSTNode<Integer> root, int value) {
        return rceil(root, value, null);
    }

    public static Integer rceil(BSTNode<Integer> root, int value, Integer currentBest) {
        if (root == null) return currentBest;
        if (root.getKey() == value) return value;
        else if (root.getKey() > value) {
            if (currentBest == null || root.getKey() < currentBest) {
                return rceil(root.getLeft(), value, root.getKey());
            } else {
                return rceil(root.getLeft(), value, currentBest);
            }
        } else {
            return rceil(root.getRight(), value, currentBest);
        }
    }
