...
public class ArrayBST<Key extends Comparable<Key>, Value> {
    ...
    public boolean put(Key key, Value val) {
        ...
        for (int current = 0; current < keys.size();) {
            int cmp = key.compareTo(keys.get(current));
            if (cmp < 0) { // On va dans la branche gauche
                if (idxLeftNode.get(current) == NONE) {
                    values.add(val);
                    keys.add(key);
                    idxLeftNode.add(NONE);
                    idxRightNode.add(NONE);
                    idxLeftNode.set(current, values.size() -1);
                    return true;
                } else {
                    current = idxLeftNode.get(current);
                }
            } else if (cmp == 0) { // On doit remplacer
                values.set(current, val);
                return false;
            } else { // Ben il reste que la branche droite dcp
                if (idxRightNode.get(current) == NONE) {
                    values.add(val);
                    keys.add(key);
                    idxLeftNode.add(NONE);
                    idxRightNode.add(NONE);
                    idxRightNode.set(current, values.size() -1);
                    return true;
                } else {
                    current = idxRightNode.get(current);
                }
            }
        }
        return false;
