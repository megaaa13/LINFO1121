...
public class RedBlackTreeConverter {
    ...
    public static<Key extends Comparable<Key>> RBNode<Key> convert(TwoThreeNode<Key> twoThreeNode) {
        ...
        if (twoThreeNode.is2node()) {
            RBNode<Key> root =  new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Black, 1);
            root.leftChild = convert(twoThreeNode.leftChild);
            root.rightChild = convert(twoThreeNode.centerChild);
            root.size += sizeEvenIfNull(root.leftChild) + sizeEvenIfNull(root.rightChild);
            return root;
        } else {
            RBNode<Key> right = new RBNode<>(twoThreeNode.rightKey, twoThreeNode.rightValue, Color.Black, 1);
            right.leftChild = new RBNode<>(twoThreeNode.leftKey, twoThreeNode.leftValue, Color.Red, 1);
            right.leftChild.leftChild = convert(twoThreeNode.leftChild);
            right.leftChild.rightChild = convert(twoThreeNode.centerChild);
            right.rightChild = convert(twoThreeNode.rightChild);
            right.leftChild.size += sizeEvenIfNull(right.leftChild.leftChild) + sizeEvenIfNull(right.leftChild.rightChild);
            right.size += sizeEvenIfNull(right.leftChild) + sizeEvenIfNull(right.rightChild);
            return right;
        }
    }
